...
# outline script:
# - function that calculates an average cross validated ROC curve, given
    # 1. a set of featues (slope_and_stemp and the number of features)
    # 2. the type of negative validation data (neg)
    # 3. the number of principal components (npcs)
    # outline function:
        # - read in all data
        # - PCA
...
#%%
# define function that calculates average cross validated ROC curve with varying number of PCs
def number_pcs(
        ...
        fs, # select features, e.g. (0, 1, 2) (0=radar, 1=velocity, 2=icethickness, 3=slope, 4=temperature, 5=distance to outcrops)
        npcs # set the number of principal components considered
        ):

    # create folder to save results
    try:
        os.mkdir('../Results/'+slope_and_stemp[0]+slope_and_stemp[1]+neg+'_variouspcs')
    ...
    def crossvalroc(data_mets, # positive observations
        ...
        # define specific negative test data ("test_neg") for two scenarios:
        ...
        # define random negative test data
        if neg == 'RandNeg':
        ...
        for b in range(0,10): #range(len(FSs)) 
            # define positive test data (test_pos)
            if FSs[b]=='rest':
            ...
            ROCvals2.to_csv('../Results/'+slope_and_stemp[0]+slope_and_stemp[1]+neg+'_variouspcs/ROC_values_CrossValidation_'+FSs[b]+'_'+version+'.csv',
                            index=False)
    ...
    # select the features defined in the function by "fs"
    columns_to_sel = [0,1] + list((fs + (2*np.ones(len(fs)))).astype(int))
    data_mets_sel = data_mets_locs.iloc[:, columns_to_sel + [-2,-1]]
    data_toclass_sel = data_toclass_transf.iloc[:, columns_to_sel]
    # define part of filename that indicates the set of features
    feature_combination_filename = ''.join(map(str,list(fs)))
    # define filename to save results
    version = str(npcs) + 'pcs' + str(len(fs)) + 'fs' + feature_combination_filename
    # perform cross validation
    crossvalroc(data_mets_sel,data_toclass_sel,cost,version,npcs)
    ...
    for b in range(len(FSs)):
        ...
        ROC_imp2 = pd.read_csv(
            '../Results/'+slope_and_stemp[0]+slope_and_stemp[1]+neg+'_variouspcs/ROC_values_CrossValidation_'+FSs[b]+'_'+version+'.csv')
    ...
    ROC_average.to_csv('../Results/'+slope_and_stemp[0]+slope_and_stemp[1]+neg+'_variouspcs/ROC_average_'+version+'.csv',
                       index=False)
    ...
    aucs_df.to_csv('../Results/'+slope_and_stemp[0]+slope_and_stemp[1]+neg+'_variouspcs/AUCs_' + 
                   str(npcs) + 'pcs' + str(len(fs)) + 'fs.csv')