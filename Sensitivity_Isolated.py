...
def crossvalroc(data_mets, # positive observations
    ...
    for b in range(0,10): #range(len(FSs)) 
        ...
        # FIRST SELECT ONLY COUNTS > 1
        data_mets_density = data_mets[data_mets.counts>1]
        print(len(data_mets_density))
        if FSs[b]=='rest':
            train_lab = data_mets_density[(data_mets_density.abbrevs=='QUE') |
                              (data_mets_density.abbrevs=='MIL') |
                              (data_mets_density.abbrevs=='LEW') |
                              (data_mets_density.abbrevs=='EET') |
                              (data_mets_density.abbrevs=='GRV') |
                              (data_mets_density.abbrevs=='ALH') |
                              (data_mets_density.abbrevs=='MAC') |
                              (data_mets_density.abbrevs=='PCA') |
                              (data_mets_density.abbrevs=='FRO')].iloc[:,:-2]
        else: 
            train_lab = data_mets_density[(data_mets_density.abbrevs!=FSs[b])].iloc[:,:-2]        
...
# plot ROC curves
# define versions
versions=['SensitivityIsolatedRandNeg','SensitivityIsolatedSpecNeg']
...
roc_isolated_rand = pd.read_csv('../Results/ROC_average_SensitivityIsolatedRandNeg.csv')
auc_isolated_rand = computeAUCs_average(roc_isolated_rand)
# export values
df_rand = pd.Series(auc_isolated_rand)
df_rand.to_csv('../Results/SensitivityIsolatedRandNeg.csv')


# compute AUC ROC negative calibration data
roc_isolated_neg = pd.read_csv('../Results/ROC_average_SensitivityIsolatedSpecNeg.csv')
auc_isolated_neg = computeAUCs_average(roc_isolated_neg)
# export values
df_neg = pd.Series(auc_isolated_neg)
df_neg.to_csv('../Results/SensitivityIsolatedSpecNeg.csv')
