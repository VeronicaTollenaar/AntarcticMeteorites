...
def exhaustive_feature_selection(
    ...
    def crossvalroc(
        ...
        ## Compute Principal Components (in case number of features equals 1, no pca is performed)
        if pcmax > 1:
            pca = PCA()
            ...
            pcs_train_unlab_st = train_unlab_st
            pcs_test_neg_st = test_neg_st
        ...
        if pcmax > 1:
            ...
            xy_train_unlab  = np.array(randsample_pcs_train_unlab_st[:,0:pcmax].tolist()).squeeze().reshape(-1, 1)
        ...
        for b in range(0,10): #range(len(FSs)) 
            ...
            if pcmax > 1:
                ...
                pcs_train_lab_st = train_lab_st
                pcs_test_pos_st = test_pos_st
            ...
            if pcmax > 1:
                ...
                xy_train_lab  = np.array(pcs_train_lab_st[:,0:pcmax].tolist()).squeeze().reshape(-1,1)
            ...
            if pcmax > 1:
                ...
                xy_test_pos  = np.array(pcs_test_pos_st[:,0:pcmax].tolist()).squeeze().reshape(-1,1)
    ...
    for nfeat in range(1,7):
        ...
        # loop over all different subsets of features
        for subset in itertools.combinations(columns_all, n_features):
            if not (1 in subset and 4 in subset): # only using subsets that are not yet calculated in "ExhaustiveFeatureSelection_CombinationsWithTemperatureVelocity.py"
